<div class="row">
	<div class="col-12">
		<div class="card">
			<form action="<?php echo $provider->route->update(consts::$page_account, $provider->user->id); ?>" method="post">
				<div class="card-body">
					<div class="mb-3">
					...
					<div class="mb-3">
						...
						<?php foreach($provider->dictionary->active() as $key => $value): ?>
							<?php if(($provider->variables->post('active') != "" && $provider->variables->post('active') == $key) || ($provider->variables->post('active') == "" && $provider->variables->data['active'] == $key)): ?>
								<input type="text" class="form-control" name="active" value="<?php echo $value; ?>" disabled>
							<?php endif; ?>
						<?php endforeach; ?>
					</div>
					<div class="mb-3">
						...
						<?php foreach($provider->dictionary->acl() as $key => $value): ?>
							<?php if(($provider->variables->post('acl') != "" && $provider->variables->post('acl') == $key) || ($provider->variables->post('acl') == "" && $provider->variables->data['acl'] == $key)): ?>
								<input type="text" class="form-control" name="acl" value="<?php echo $value; ?>" disabled>
							<?php endif; ?>
						<?php endforeach; ?>
					</div>
					<div class="mb-3">
